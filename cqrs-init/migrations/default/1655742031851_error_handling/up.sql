...
CREATE VIEW public.commands_responses AS
 SELECT DISTINCT ON (c.item_id) c.cmd_id,
    c.cmd_type,
    c.cmd_ref,
   ...
   FROM (public.commands c
     LEFT JOIN ( SELECT responses.cmd_id,
          ...
          ORDER BY responses.cmd_id, responses.created_at DESC) r ON ((c.cmd_id = r.cmd_id)))
  WHERE (c.created_at > (now() - '08:00:00'::interval))
  ORDER BY c.item_id DESC, c.created_at DESC;
